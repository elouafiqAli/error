...
$d_{\mathcal{H}_{\ell}}$, and call an MPNN family
\emph{$\delta_{0}$-quasi-admissible} if its layer maps
$\phi_{\ell}$ are $L_{\ell}$-Lipschitz under
$d_{\mathcal{H}_{\ell-1}} \to d_{\mathcal{H}_{\ell}}$
(jointly in the central argument and in the Wasserstein-$1$
lift of the neighbour multiset, with a fixed permutation-
invariant aggregator) and the initial feature map satisfies
$d_{\mathcal{H}_{0}}(h^{(0)}(v), h^{(0)}(w)) \le \delta_{0}$
whenever $v, w$ share a cell of the \emph{ideal}
(constant-feature) $1$-WL partition.
...
For every member of $\mathcal{A}$ and every $v, w \in V$
sharing a cell of the ideal $\mathrm{WL}_{L}(G)$,
\[
...
\begin{proof}
The distance bound is a one-line induction on $\ell$ using the
joint Lipschitz hypothesis on $\phi_{\ell}$ and the
Wasserstein-$1$ bound on the multiset of neighbour states
(triangle inequality across $|N(v)| \le \Delta$ terms gives the
$(1 + \Delta)$ factor). The risk bound then follows because
$\tau$ collapses $\mathcal{H}_{L}$-balls of radius $\delta_{L}$
to $[0,1]$-intervals of length $\le L_{\tau} \delta_{L}$, and a
threshold rule on such an interval can flip at most
$L_{\tau} \delta_{L}$ mass per cell relative to the partition-
constant optimum.
\end{proof}
...
cost of a slack term $L_{\tau} \delta_{L}$ that grows
exponentially in $L$. The interpretation is intentionally
