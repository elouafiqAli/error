...
def run_all(seeds=(0, 1, 2, 3, 4), L=3, hidden=128, epochs=200,
    ...
    for name in datasets:
        try:
            ...
            err = {"name": name, "error": f"{type(e).__name__}: {e}"}
            out["datasets"].append(err)
            print(f"  [!] {name} FAILED: {err['error']}", flush=True)
        # ---- per-dataset checkpoint: write JSON after EVERY dataset
        out["total_wall_s"] = time.perf_counter() - t0
        out_path.write_text(json.dumps(out, indent=2))
        print(f"  [checkpoint] wrote {out_path}  "
              f"({len([d for d in out['datasets'] if 'runs' in d])}/"
              f"{len(datasets)} datasets ok)", flush=True)
