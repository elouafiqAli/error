"""HW1 — Binary entropy and Hellman–Raviv on a single coin.

Regenerates both `hw1.ipynb` (student) and `hw1_solution.ipynb`
(reference) from the cell list below. Run from the repo root:

    cd onboarding/projects && python -m scaffold.hw1
"""
from __future__ import annotations

from pathlib import Path

from onboarding.projects.scaffold import Cell, write_pair, execute, _setup_cells
...
HW1_CELLS: list[Cell] = [
    ...
    *_setup_cells("hw1"),
    ...
    Cell("md",
         "## Q1 — Binary entropy\n\n"
         "**Concept.** $H_{\\mathrm{bin}}(p) = -p \\log_2 p - (1-p)\\log_2(1-p)$, "
         "in bits, with $0 \\log_2 0 := 0$. Four properties to demonstrate "
         "numerically before proving in the writeup:\n\n"
    ...
    Cell("md",
         "**Distinguish — concavity vs unimodality.** $H_\\mathrm{bin}$ is "
         "both concave *and* unimodal with peak at $1/2$; unimodality alone "
         "would not imply Hellman–Raviv. We overlay the secant chord from "
         "$(0.1, H(0.1))$ to $(0.9, H(0.9))$ to make concavity visible."),
    Cell("demo",
    ...
    Cell("md",
         "**Gate Q1.** Four invariants — each checked numerically below. "
         "If any assertion fires, the printed message names the failing property."),
    Cell("gate",
    ...
    Cell("md",
         ...
         "**Concept.** For $Y \\sim \\mathrm{Bernoulli}(p)$, the "
         "minimum-error deterministic classifier predicts the majority "
         "class and incurs $\\varepsilon(p) = \\min(p, 1-p)$. "
         "Randomised classifiers cannot beat this (Jensen on 0/1 loss)."),
    Cell("solution",
         "def bayes_error(p: float) -> float:\n"
         "    \"\"\"Bayes error of a Bernoulli(p): ε(p) = min(p, 1-p).\"\"\"\n"
         "    return min(p, 1 - p)\n"),
    Cell("md",
         "**Distinguish — `bayes_error` vs `hbin`.** Both vanish at the "
         "endpoints and peak at $1/2$, but $\\varepsilon$ is *piecewise "
         "linear* (kinked at $1/2$) whereas $H_\\mathrm{bin}$ is *smooth "
         "and concave*. The slack of HR rides on this distinction."),
    Cell("demo",
    ...
    Cell("md",
         "**Gate Q2.** $\\varepsilon(p) \\le 1/2$ everywhere; "
         "$\\varepsilon(p) = \\varepsilon(1-p)$ (symmetry); spot-check $\\varepsilon(0.3) = \\varepsilon(0.7) = 0.3$."),
    Cell("gate",
    ...
    Cell("md",
         ...
         "**Concept.** $\\varepsilon(p) \\le \\tfrac{1}{2} H_\\mathrm{bin}(\\varepsilon(p))$ "
         "for all $p \\in [0, 1]$. We compute LHS, RHS, slack on a grid, "
         "count violations, and locate the maximum slack.\n\n"
         "**Calibration tag the paper uses for $w^*$:** $w^* \\approx 0.1610$, "
         "attained at $\\varepsilon^* = 1/5$, i.e. at $p^* = 1/5$ or $4/5$."),
    Cell("solution",
         "def hr_violations(grid: np.ndarray, atol: float = 1e-12) -> dict:\n"
         "    \"\"\"Audit HR on a grid of p values.\n"
         "\n"
         "    Returns {n, violations, max_slack, argmax_p}.\n"
         "    \"\"\"\n"
         "    eps_vals = np.array([bayes_error(p) for p in grid])\n"
         "    lhs = eps_vals\n"
         "    rhs = 0.5 * np.array([hbin(e) for e in eps_vals])\n"
         "    slack = rhs - lhs\n"
         "    violations = int(np.sum(lhs > rhs + atol))\n"
         "    idx = int(np.argmax(slack))\n"
         "    return {\n"
         "        'n': len(grid),\n"
         "        'violations': violations,\n"
         "        'max_slack': float(slack[idx]),\n"
         "        'argmax_p': float(grid[idx]),\n"
         "    }\n"),
    Cell("md",
         "**Distinguish — single-coin slack vs partition $w^*$.** Q3 measures "
         "the slack of HR on a *single coin*; the paper's "
         "$w^* \\approx 0.1610$ is slack at the maximum over fixed-$H$ "
         "*partitions*. They coincide numerically because the single-coin "
         "sweep happens to traverse the same upper envelope."),
    Cell("demo",
    ...
    Cell("md",
         "**Gate Q3.** Rubric: `violations == 0`, `0.10 < max_slack < 0.20`, "
         "and the *Bayes error* at the argmax is approximately $1/5$ "
         "(slack is symmetric about $p = 1/2$, so `argmax_p` itself may "
         "be either $\\approx 1/5$ or $\\approx 4/5$). Tighter spot-check: "
         "`max_slack ≈ 0.1610`."),
    Cell("gate",
    ...
    Cell("md",
         ...
         "**Concept.** A verifier you cannot break with a *deliberately wrong* "
         "predicate is a rubber stamp. We design three single-line "
         "mutations of the HR predicate:\n\n"
         "- **A** — shrink the $\\tfrac{1}{2}$ factor to $0.4$. (Breaks "
         "  Hellman–Raviv's slack margin → SHOULD violate.)\n"
         "- **B** — replace $\\varepsilon$ with $\\varepsilon^2$ inside "
         "  $H_\\mathrm{bin}$. (Breaks the proof's curvature step → "
         "  SHOULD violate.)\n"
         "- **C** — equality case: set LHS := RHS. (The verifier must "
         "  *not* fire false positives on the boundary → SHOULD hold.)"),
    Cell("solution",
         "def mutation_A(grid: np.ndarray, atol: float = 1e-12) -> dict:\n"
         "    \"\"\"Factor 1/2 → 0.4. Should violate.\"\"\"\n"
         "    eps = np.array([bayes_error(p) for p in grid])\n"
         "    lhs = eps\n"
         "    rhs = 0.4 * np.array([hbin(e) for e in eps])\n"
         "    excess = lhs - rhs\n"
         "    v = int(np.sum(excess > atol))\n"
         "    i = int(np.argmax(excess))\n"
         "    return {'n': len(grid), 'violations': v,\n"
         "            'max_excess': float(max(0.0, excess.max())),\n"
         "            'argmax_excess_p': float(grid[i])}\n"
         "\n"
         "def mutation_B(grid: np.ndarray, atol: float = 1e-12) -> dict:\n"
         "    \"\"\"Replace ε with ε² inside H_bin. Should violate (kills curvature).\"\"\"\n"
         "    eps = np.array([bayes_error(p) for p in grid])\n"
         "    lhs = eps\n"
         "    rhs = 0.5 * np.array([hbin(e * e) for e in eps])\n"
         "    excess = lhs - rhs\n"
         "    v = int(np.sum(excess > atol))\n"
         "    i = int(np.argmax(excess))\n"
         "    return {'n': len(grid), 'violations': v,\n"
         ...
         "            'argmax_excess_p': float(grid[i])}\n"
         "\n"
         "def mutation_C(grid: np.ndarray, atol: float = 1e-12) -> dict:\n"
         "    \"\"\"Negative control: LHS := RHS. Verifier must not fire.\"\"\"\n"
         "    eps = np.array([bayes_error(p) for p in grid])\n"
         "    rhs = 0.5 * np.array([hbin(e) for e in eps])\n"
         "    lhs = rhs.copy()\n"
         "    excess = lhs - rhs\n"
         "    v = int(np.sum(excess > atol))\n"
         "    return {'n': len(grid), 'violations': v,\n"
    ...
    Cell("md",
         "**Distinguish — falsification, not confirmation.** A test suite "
         "that only contains positive examples (\"this should pass\") "
         "doesn't prove the test machinery works. Mutation C is the "
         "*negative control*: we deliberately set LHS = RHS and confirm "
         "the predicate `lhs > rhs + atol` does *not* trip."),
    Cell("demo",
         "grid = np.linspace(0, 1, 10_001)\n"
    ...
    Cell("md",
         "**Gate Q4.** A and B fire; C does not. Slack at A's worst-case "
         "should be at least $0.01$; B's at least $0.05$."),
    Cell("gate",
    ...
    Cell("md",
         ...
         "Per rubric: at least 5 calibration entries in `writeup.md`. The "
         "`reflect.log(...)` calls above (Q1–Q4) cover the assertion gates. "
         "This cell records the writeup itself."),
    Cell("reflect",
...
def main() -> None:
    out = Path(__file__).resolve().parent.parent / "psets" / "hw1"
    sol, stu = write_pair(HW1_CELLS, out_dir=out, stem="hw1")
