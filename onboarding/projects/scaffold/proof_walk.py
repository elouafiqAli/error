"""HW3 companion — line-by-line walk of Appendix A.

Regenerates psets/hw3/proof_walk.ipynb. Each Appendix-A step is paired
with a numeric assertion on a 5000-point grid so the algebraic
manipulation is *audited*, not just narrated.

    cd onboarding/projects && python -m scaffold.proof_walk
"""
...
CELLS: list[Cell] = [
    Cell("md",
         ...
         "**Source.** [`PAPER-ARXIV.md`](../../../../PAPER-ARXIV.md) Appendix A "
         "(proof of Theorem 1, partition-conditional binary-entropy bracket).\n\n"
         "**Doctrine.** Every algebraic step gets a numeric witness on a "
         "5 000-point grid; if you can't certify the step in `float64`, you "
         "haven't understood it. We *do not* re-derive `hbin`, `hbin_inverse`, "
         "`upper_HR`, or `lower_Fano` — HW3 already did. We only *use* them."),
    *_setup_cells("hw3.proof_walk"),
    Cell("demo",
    ...
    Cell("md",
         "## A.1 — Setup\n\n"
         "Fix binary $Y \\in \\{0, 1\\}$ and a finite measurable partition "
         "$\\Pi = \\{C_1, \\ldots, C_m\\}$ of the input. Write "
         "$q_C = \\Pr(C)$ and $p_C = \\Pr(Y=1 \\mid C)$. The "
         "**per-cell Bayes error** is $e_C = \\min(p_C, 1-p_C) \\in [0, 1/2]$. "
         "The **partition-conditional Bayes error** is\n\n"
    ...
    # --- A.4 Jensen variant for tightness ---------------------------------
    Cell("md",
...
def main(root: Path | None = None) -> None:
    ...
    write_pair(CELLS, out, stem="proof_walk")
