...
H_prime_fn = build_function(H_prime_sym, ps; expression=Val{false})
...
let
    H_dd_fn = build_function(H_double_sym, ps; expression=Val{false})
    qs = 0.05:0.01:0.95
...
@bind base_choice Select([2 => "2 (bits)", exp(1) => "e (nats)", 3 => "3 (trits)", 10 => "10 (digits)"])
